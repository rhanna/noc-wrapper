 ...
 *   - Accept a pre-established session/cookie state at construction or per-call
 *   - Throw descriptive errors that upper layers can act on — e.g. SessionExpiredError, AuthFailedError, ParseError — not generic fetch errors
 *   - Be stateless between calls beyond the cookie jar it manages
 *   - Have no retry logic, no enrichment, no queuing
 * The rich error types are the most important contract NocBrowser provides upward. NocWrapper needs to know why something failed to decide whether to retry or re-auth. If NocBrowser just throws Error: fetch failed, the upper layers are flying blind.
 * 
